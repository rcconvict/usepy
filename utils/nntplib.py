...
if _have_ssl:
    ...
    def _encrypt_on(sock, context):
        ...
        """
		# This really needs some improving
		# Check to see what ssl versions are available on server
		# Use sslv23 for now
        try:
			# Just wrap the socket with SSLv23, SSLv2 is harmful apparently
            return ssl.wrap_socket(sock, ssl_version=ssl.PROTOCOL_SSLv23)
        except ssl.SSLError:
			# Need to check if this is actually the error, SSLError holds
			# a lot of errors.
            raise NNTPProtocolError('Port does not seem to be a valid SSL port.')
